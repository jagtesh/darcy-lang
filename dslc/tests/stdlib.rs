...
fn core_vec_len_lowers() {
    let src = "(defn main [xs:vec<i32>] (darcy.vec/len xs))";
...
fn core_vec_is_empty_lowers() {
    let src = "(defn main [xs:vec<i32>] (darcy.vec/is-empty xs))";
    let out = compile(src).expect("compile ok");
...
fn core_num_min_max_clamp_lowers() {
    let src = "(defn main [x:i32 y:i32 z:i32] (darcy.math/clamp (darcy.math/max x y) 0 z))";
    let out = compile(src).expect("compile ok");
...
fn core_num_abs_lowers() {
    let src = "(defn main [x:i32] (darcy.math/abs x))";
    let out = compile(src).expect("compile ok");
