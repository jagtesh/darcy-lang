...
fn infers_struct_field_types() {
    let src = "(defrecord point [x] [y]) (defn get-x [p] p.x) (defn main [] (get-x (point 1 2)))";
...
fn infers_union_field_types_from_match() {
    let src = "(defenum opt (some [v]) (none)) (defn get [o] (case o (some (v v) v) (none 0))) (defn main [] (get (some 1)))";
    let out = compile(src).expect("expected inference to succeed");
