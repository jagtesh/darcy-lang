...
fn core_str_len_is_empty_lowers() {
    let src = "(defn main [s:string] (core.str/is-empty s))";
...
fn core_str_trim_lowers() {
    let src = "(defn main [s:string] (core.str/trim s))";
    let out = compile(src).expect("compile ok");
...
fn core_str_split_join_lowers() {
    let src = "(defn main [s:string sep:string] (core.str/join (core.str/split s sep) \"|\"))";
    let out = compile(src).expect("compile ok");
