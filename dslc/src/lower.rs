...
pub fn lower_program(pipeline: &PipelineOutput) -> DslResult<String> {
    ...
    let mut top_fn_names = BTreeSet::new();
    for t in &pipeline.tops {
        if let Top::Func(fd) = t {
            top_fn_names.insert(fd.name.clone());
        }
    }
    let mut typed_fn_map: BTreeMap<String, &TypedFn> = BTreeMap::new();
    for f in &pipeline.typechecked.typed_fns {
        typed_fn_map.insert(f.def.name.clone(), f);
    }
    for t in &pipeline.tops {
        if let Top::Func(fd) = t {
            let typed = match typed_fn_map.get(&fd.name) {
                Some(v) => *v,
                None => continue,
            };
            if fd.extern_ {
            ...
            out.push_str(&format!("{}fn {}{}(", vis, fd.rust_name, generics));
    ...
    for f in &pipeline.typechecked.typed_fns {
        if top_fn_names.contains(&f.def.name) {
            continue;
        }
        if f.def.extern_ {
            continue;
    ...
    let mut inner = match e {
        ...
        Expr::Call { op, args, span } => {
                ...
                .contains(&op.as_str())
            {
                ...
                let modes = fn_env.get(op).map(|sig| sig.param_modes.clone());
